...
One caveat if you package for the Store: at 30 to 50px the mascot is legible as
a creature, but its eyes and craft knife are very soft. `Square30x30Logo` sits
in that band. If the small Start tile looks muddy in practice, the fix is to
raise the C-mark threshold in `build-icons.py` above 30 rather than to redraw
anything.
