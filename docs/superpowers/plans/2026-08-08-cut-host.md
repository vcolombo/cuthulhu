...
**Interfaces:**
- Consumes: `cutplan::preflight::{PreflightError}`, `driver_core::{MachineProfile, MachineCaps, Settings}`, `driver_core::manager::CutPass`, `geometry::affine::Point`.
...
- [ ] **Step 3: Make the host reachable from an integration test**

Three changes let the fixture above compile. All three are in library code, none in the tests:

1. `Host::testing` must be reachable from an integration test. In `crates/cut-host/src/host.rs`, change `#[cfg(test)] pub(crate) mod testing` to:

```rust
/// Mock cutters. Public because the integration tests in `tests/` are a separate
/// crate and cannot reach a `#[cfg(test)]` module.
pub mod testing {
```

(dropping the `#[cfg(test)]` attribute, keeping the module body unchanged).

2. Split the listener out of `serve`, so a test can bind port 0 and learn the port. Replace `serve`'s body's last section — everything from `let listener = TcpListener::bind(config.bind)?;` — with a call, and add the new function:
...
/// Serve on a listener somebody else bound. Split out so a test can take port 0
/// and read back what the OS gave it.
...
/// yet. What a test — and `cuthulhu-cutd`'s startup line — reads to pair.
...
`serve` keeps its public-bind guard and its fingerprint print; only the loop moves.

3. In `crates/cut-host/Cargo.toml`, `tempfile` is already a dev-dependency from Task 7 — no change.
